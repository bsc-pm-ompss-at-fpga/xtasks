...
/*!
 * \brief Get the number of available accelerators in the specified device
 */
xtasks_stat xtasksGetNumAccs(int devId, size_t *count);

/*!
 * \brief Get the accelerator handles for the accelerators in the specified device
 ...
 * \param[in]  parent     Parent task identifier (it can be NULL if not known or not needed)
 ...
 * \returns    XTASKS_SUCCESS if the action has been successfully completed
 */
 ...
 *             XTASKS_SUCCESS if the copy operation has successfully finalized
 */
