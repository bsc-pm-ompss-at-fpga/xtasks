...
{
    if (_instrBuff == NULL) return XTASKS_ERROR;
    free(_instrBuff);
...
xtasks_stat xtasksInit()
{
    ...
    admxrc3_status =
        ADMXRC3_MapWindow(_hDevice, 1, CMD_IN_QUEUE_ADDR, sizeof(uint64_t) * CMD_IN_QUEUE_LEN, (void **)&_cmdInQueue);
    if (admxrc3_status != ADMXRC3_SUCCESS) {
        ret = XTASKS_EFILE;
    ...
    admxrc3_status = ADMXRC3_MapWindow(
        _hDevice, 1, CMD_OUT_QUEUE_ADDR, sizeof(uint64_t) * CMD_OUT_QUEUE_LEN, (void **)&_cmdOutQueue);
    if (admxrc3_status != ADMXRC3_SUCCESS) {
        ret = XTASKS_EFILE;
    ...
    if (feature == BIT_FEATURE_NO_AVAIL)
        ...
        admxrc3_status = ADMXRC3_MapWindow(
            _hDevice, 1, SPAWN_OUT_QUEUE_ADDRESS, sizeof(uint64_t) * SPWN_OUT_QUEUE_LEN, (void **)&_spawnOutQueue);
        if (admxrc3_status != ADMXRC3_SUCCESS) {
            ret = XTASKS_EFILE;
    ...
    if (feature == BIT_FEATURE_NO_AVAIL)
        ...
        admxrc3_status = ADMXRC3_MapWindow(
            _hDevice, 1, SPAWN_IN_QUEUE_ADDRESS, sizeof(uint64_t) * SPWN_IN_QUEUE_LEN, (void **)&_spawnInQueue);
        if (admxrc3_status != ADMXRC3_SUCCESS) {
            ret = XTASKS_EFILE;
...
INIT_ERR_MMAP_RST:
    if (_spawnInQueue != NULL) ADMXRC3_UnmapWindow(_hDevice, _spawnInQueue);
INIT_ERR_MAP_REMFINI:
    if (_spawnOutQueue != NULL) ADMXRC3_UnmapWindow(_hDevice, _spawnOutQueue);
INIT_ERR_MAP_NEW:
...
xtasks_stat xtasksFini()
{
    ...
    if (_spawnOutQueue != NULL) statusNw = ADMXRC3_UnmapWindow(_hDevice, _spawnOutQueue);
    if (_spawnInQueue != NULL) statusRFi = ADMXRC3_UnmapWindow(_hDevice, _spawnInQueue);
...
xtasks_stat xtasksTryGetNewTask(xtasks_newtask **task)
{
    ...
    taskSize = (sizeof(new_task_header_t) + sizeof(uint64_t) * numArgs + sizeof(new_task_dep_t) * numDeps +
    ...
    *task = realloc(*task, sizeof(xtasks_newtask) + sizeof(xtasks_newtask_arg) * numArgs +
                               sizeof(xtasks_newtask_dep) * numDeps + sizeof(xtasks_newtask_copy) * numCopies);
...
#ifdef XTASKS_DEBUG
uint64_t cmdInQueue(size_t const accID, size_t const idx) { return _cmdInQueue[accID * CMD_IN_SUBQUEUE_LEN + idx]; }

uint64_t cmdOutQueue(size_t const accID, size_t const idx) { return _cmdOutQueue[accID * CMD_OUT_SUBQUEUE_LEN + idx]; }
...
void accelPrintInstrBuffer(size_t const aIdx)
{
    ...
    xdma_status stat = xdmaMemcpy(event, _instrBuffHandle,
        (_numAccs * _numInstrEvents - aIdx * _numInstrEvents) * sizeof(xtasks_ins_event),
        (aIdx * _numInstrEvents) * sizeof(xtasks_ins_event), XDMA_FROM_DEVICE);
    if (stat != XDMA_SUCCESS) return;
